...
September 20 consolidation: the original idea lineage below is preserved. The current developed proposal set is Structural Research Directions (deleted draft), and the maintained field synthesis is the [general literature review](UAV%20General%20Literature%20Review%20-%202026-09-20.md). Older “current” agenda statements below describe earlier stages of the research.

September 20 clarification: the revised Handoff architecture (deleted draft) develops the original JEV-like visual decision model, action-conditioned JEPA, shared fast/slow VLM–VLA, and geometric/Gaussian ideas as questions about model state, training, and control. It supersedes interface-first Handoff framing while preserving the original ideas below. [FlowPilot](https://arxiv.org/html/2608.00635v1) is now an essential close baseline for fast UAV world-action modeling.

## Purpose of this document

**Current method proposals: CURI-UAV method development, September 19 (deleted draft).** The mission-only test agenda drifted from the intended purpose of inventing and implementing research methods. The revised proposals develop the original local visual decision model, geometric JEPA/shared representation and rapid reconstruction interests into concrete learning/information-flow mechanisms. Navigation, tracking and exploration remain the application map. The original ideas below remain traceable; proposed extensions are labelled separately. Novelty remains unresolved, and no trained full method is claimed from starter operator tests.
