...
class CausalMetricAlignment:
    ...
    def update(self,episode_id,observation_ns,available_ns,gauge_version,map_position,metric_position,
        ...
        return self._fit(observation_ns,available_ns,gauge_version)

    def update_prefix(self,episode_id,observation_ns,available_ns,gauge_version,pairs,now_ns):
        """Fit one published map revision against historical causal VO.

        Bundle adjustment revises old map cameras. Reusing their superseded
        poses in a similarity fit mixes different coordinate estimates and
        need not describe the geometry being handed to navigation.
        """
        if episode_id!=self.episode_id or not observation_ns<=available_ns<=now_ns:
            raise ValueError('Wrong episode or future alignment prefix')
        checked=[];previous=-1
        for stamp,map_position,metric_position,sigma in pairs:
            if not previous<stamp<=observation_ns:
                raise ValueError('Prefix camera timestamps must be unique, increasing and causal')
            values=np.r_[map_position,metric_position,sigma]
            if not np.isfinite(values).all() or sigma<0:
                raise ValueError('Invalid alignment prefix estimate')
            checked.append((stamp,np.asarray(map_position),np.asarray(metric_position),sigma));previous=stamp
        self.pairs.clear();self.pairs.extend(checked);self.version=gauge_version
        result=self._fit(observation_ns,available_ns,gauge_version)
        result['pose_support']='one published optimized-map camera revision'
        return result

    def _fit(self,observation_ns,available_ns,gauge_version):
        if len(self.pairs)<8:return dict(usable=False,reason='insufficient_causal_support',support=len(self.pairs))
        ...
        if eigen[0]<1e-4 or eigen[1]/eigen[0]<.05 or np.linalg.norm(yc,axis=1).max()<1:
            return dict(usable=False,reason='insufficient_motion_excitation',support=len(self.pairs))
