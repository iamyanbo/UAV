...
def build(root, output, initialization=None):
    ...
    for receipt in sorted(root.rglob('episode/result.json')):
        ...
        try:
            ...
            label_path.write_text(json.dumps(dict(frames=aligned,termination=result.get('termination'),
                ...
                teacher='privileged_shortest_path_upper_bound' if expert else 'learned_policy_rollout',
    ...
    manifest=dict(schema='visual-trajectory-bundle/v1',action_semantics='post-safety-dispatch/50ms-v3',teacher_version='observed-exploration/v3',collection_root=str(root),attempts=attempts,
