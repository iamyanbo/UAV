...
def main():
    ...
    for name in ('world','policy','projection','safety','qwen','configuration','vision'):parser.add_argument('--'+name,type=Path)
    ...
    for role in ('goal','odometry','world','policy','projection','safety','qwen','configuration','vision'):
    ...
    for role in ('goal','odometry','world','policy','projection','safety','qwen','configuration','vision'):
        ...
        target=args.output/(role+('.json' if role in ('safety','configuration','vision') else '.pt'));shutil.copyfile(source,target)
