...
def main():
    ...
    with torch.inference_mode():
        for row in records['train']:
            ...
            data=load_record(data_root,row,hashes)['runtime'];episode=row['episode_id']
            ...
            scores.append(dict(episode_id=episode,frame_id=row['frame_id'],probability=score))
    selected=[];hard=set()
    for episode,group in scored.items():
        ranked=sorted(group,key=lambda item:(-item[0],item[1]['frame_id']))
        mined=[r for _,r in ranked[:16]]
        hard.update((r['episode_id'],r['frame_id']) for r in mined)
        remaining=[r for _,r in sorted(group,key=lambda item:item[1]['frame_id']) if (r['episode_id'],r['frame_id']) not in hard]
        selected+=mined+spaced(remaining,16)
    far_keys={(r['episode_id'],r['frame_id']) for r in selected}
    ...
    manifest.update(windows=windows,negative_mining=dict(source_manifest_sha256=sha,
        ...
        selection='top 16 false match scores per training task plus 16 temporally spaced remaining far negatives'))
