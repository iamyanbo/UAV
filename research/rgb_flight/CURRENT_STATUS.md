Latest functional check (2026-09-24, 06:40 UTC): R71 completed the historical-RGB Qwen update and combined Mode 1/Qwen/planner simulator reload without a controller exception. The flight travelled 13.79 m and received eight supported Gaussian publications. Qwen received three real historical images and observed target IDs in its fourth call; that result and the planner finished too late for control. Earlier invalid abstentions were rejected. Metric initialization still timed out. R62's ten development flights all moved (4.35-15.08 m), with nine initialization timeouts and one retained collision. Balanced odometry is checkpointed at update 633 and resumes exactly toward 2,000. R72 waits for the improved perception cycle, then recollects demonstrations and runs the compatible full learning flow and cumulative continuation. Navigation and cumulative training are not complete. Timing optimization remains secondary.

The bounded receipt observer publishes execution snapshots to [OVERNIGHT_STATUS.json](OVERNIGHT_STATUS.json); it does not infer success, launch research or extend the eight-hour training window.

# Navigation-first learning repair - September 24, 2026

See [the failure postmortem](POSTMORTEM_LEARNING_LOOP_2026-09-24.md). Bounded exploration, dispatched-command learning, explicit masked belief inputs, optimized Gaussian publication, separate Qwen/planning workers and ten-demonstration preparation are implemented. Verification is in progress; this is not deployment acceptance. The user now prioritizes functional flight/navigation over the 50 ms optimization target; measured latency remains reported.
...
Round r48 completed the combined Qwen/planner reload and all ten development flights. Every development flight moved (4.87 to 14.78 m): nine initialization timeouts and one retained geometry collision. Map handover and full timing passed in zero flights. One matched configuration pair tied with no control exposure; preference learning is explicitly blocked. [Implementation, measured limits and continuation](LEARNING_REPAIR_2026-09-24.md) is the current detailed report. The corrected v4 world objective reached update 187 through verified checkpoint continuation. Another compatible r56 flow completed world, policy/critic, Qwen supervised, DAgger and fresh PPO updates (360 eligible PPO transitions) and updated-policy flights. Metric initialization still failed. The primary branch is now `navigation-repair-r60`: balanced odometry training on expanded recordings, recorded qualification, and new perception-only teacher flights. `navigation-repair-r62` waits for complete demonstrations, then runs compatible learner updates, fresh physical DAgger/PPO, final reloads, development flights and eligible cumulative continuation. Neither pending work nor update budgets are reported as completed.
