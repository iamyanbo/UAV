Latest functional check: R60 completed 200 odometry updates and full recorded qualification, but odometry drift remains too high (4 s translation RMSE 6.25 m). Its panoramic pilot flew 11.97 m and timed out before metric map handover. R63 verifies incremental Gaussian publication because the prior mapper first published after the flight ended. R62 waits for the complete v4 demonstration batch before the complete new-perception learning flow. Training and navigation are not complete or deployment accepted. Timing optimization is secondary under the user's latest steering.
