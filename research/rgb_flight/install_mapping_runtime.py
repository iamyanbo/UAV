...
parser.add_argument('--metric-depth',action='store_true',help='Add pure-Python Metric3D dependencies without replacing existing OpenCV')
...
dependencies=('pip install --no-cache-dir --no-deps mmengine==0.10.7 mmcv-lite==2.2.0 addict==2.4.0 yapf==0.43.0 termcolor==3.3.0'
              if args.metric_depth else 'apt-get update && apt-get install -y --no-install-recommends libgl1 libegl1 && apt-get clean')
...
receipt = dict(previous, image_id=image, parent_image_id=previous['image_id'],
               runtime_dependency=dependencies, dockerfile_sha256=hashlib.sha256(dockerfile.read_bytes()).hexdigest())
